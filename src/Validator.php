...
class Validator
{
    ...
    public function validate($authorizationHeader)
    {
        if (is_null($authorizationHeader)) {
            throw new TokenException('no_token', 'no authorization header', 401);
        }
        ...
        if (1 !== preg_match('|^Bearer [a-zA-Z0-9-._~+/]+=*$|', $authorizationHeader)) {
            throw new TokenException('no_token', 'no Bearer token', 401);
        }

        $bearerToken = substr($authorizationHeader, 7);
        if (false === strpos($bearerToken, '.')) {
            throw new TokenException('invalid_token', 'bearer token is expected to contain a "."', 401);
        }

        list($accessTokenKey, $accessToken) = explode('.', $bearerToken);
        $tokenInfo = $this->tokenStorage->getToken($accessTokenKey);
        if (false === $tokenInfo) {
            throw new TokenException('invalid_token', 'token key does not exist', 401);
        }
        ...
        if ($this->dateTime > $expiresAt) {
            throw new TokenException('invalid_token', 'token expired', 401);
        }
